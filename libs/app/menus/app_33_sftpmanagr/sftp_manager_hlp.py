...
def send_key_by_mail(cfg: Dict, username: str, key: str) -> Tuple[bool, Optional[str]]:
    ...
    subject = f"SFTP key export for user: {username}"
    if priv:
    ...
    body_lines.extend([
        "",
        f"Generated by SFTP Manager - SysApp v{getattr(app_cfg, 'VERSION', '')}.",
    ])
    text_body = "\n".join(body_lines)
    ...
    html_lines = [
        "<html>",
        "  <body>",
        "    <p>SFTP key export for user: <strong>{}</strong><br>".format(html.escape(username)),
        "       Recipients: {}</p>".format(html.escape(", ".join(recipients))),
        "    <p>Public key:</p>",
    ...
    if priv:
        html_lines.extend([
            ...
            "    <p>This message contains a private key. Handle it carefully.</p>",
        ])
    else:
        html_lines.append("    <p>No private key is stored for this entry.</p>")
    html_lines.extend([
        "    <p>Generated by SFTP Manager - SysApp v{}.</p>".format(html.escape(getattr(app_cfg, "VERSION", ""))),
        "  </body>",
        "</html>",
    ])
    html_body = "\n".join(html_lines)
